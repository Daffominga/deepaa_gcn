...
class resGCNConv(MessagePassing):
    ...
    def forward(self, x: Tensor, edge_index: Adj, alpha, gamma, h0, beta, lx, hp0,
        ...
        support = alpha * x + beta * torch.matmul(x, self.weight1)  # DeepHpGCN
        ...
        # reshp = alpha * lx + beta * torch.matmul(lx, self.weight2)
        reshp = beta * torch.matmul(lx, self.weight2) + (1 - beta) * hp0

        initial = (1 - beta) * h0  # H0 is more important when layers are deep   # DeepHpGCN
