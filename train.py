...
# root='E:/workspace/pythonProject/datasets/Amazon/Computers'
# path = osp.join('E:/workspace/pythonProject/datasets', dataset)
# dataset = Planetoid(path, dataset, split="public", transform=T.NormalizeFeatures())

dataset = Planetoid(root='E:/workspace/pythonProject/datasets', name=dataset,
...
class resGCNmodel(torch.nn.Module):
    ...
    def forward(self):
        ...
        for i, con in enumerate(self.convs[1: -1]):  # such as (1, 'conv_i')
            ...
            x, lx = con(x, edge_index, alpha, gamma, _hidden[0], beta, lx, _hidden_hp[0], edge_weight)
            x = F.relu(x)
            lx = F.relu(lx)
...
for epoch in range(1, 1000):
    ...
    if bad_counter == patience:
        # w = model.convs[1].weight1
        # res = w.cpu().detach().numpy()
        # res = np.array(res)
        # np.savetxt('npresult1.txt', res)
        # io.savemat('save.mat', {'result1': res})
        break
